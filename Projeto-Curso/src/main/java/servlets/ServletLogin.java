...
/* O chamado Controller são as servlests*/
		...
		try {
				if (login != null && !login.isEmpty() && senha != null && !senha.isEmpty()) {
					...
					if (daoLoginRepository.validarAutenticacao(modelLogin)) { // simulando o login
						...
						RequestDispatcher redirecioanar = request.getRequestDispatcher(url);
						redirecioanar.forward(request, response);
		
					} else {
						RequestDispatcher redirecioanar = request.getRequestDispatcher("/index.jsp");
						request.setAttribute("msg", "Informe o login e senha correto!");
						redirecioanar.forward(request, response);
					}
